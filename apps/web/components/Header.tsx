...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'border-b border-brand-border bg-brand-background/95 shadow-[0_4px_20px_rgba(26,26,26,0.06)]' +
              (menuOpen ? '' : ' backdrop-blur-[10px]')
    ...
    {/* Mobile drawer — sibling of <header> so its `fixed inset-0` always
        resolves against the viewport. When `scrolled` the header gets a
        backdrop-filter, which would otherwise make the header the containing
        block for a fixed-position descendant. */}
    <div
        ...
        <nav aria-label="Mobile" className="flex-1 overflow-y-auto px-4 py-4">
          <ul className="flex flex-col">
            {NAV_LINKS.map((link, index) => (
              <li
                key={link.href}
                className={menuOpen ? 'animate-menu-link' : undefined}
